...
class GroupedListView<E, G extends Comparable<Object>> extends StatefulWidget {
  ...
  /// The threshold value to change the sticky header when scrolling the list
  final double? headerChangeThreshold;

  @override
  State<GroupedListView<E, G>> createState() => _GroupedListViewState<E, G>();
}
...
class _GroupedListViewState<E, G extends Comparable<Object>>
  ...
  Widget _buildListView() => ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          final actualIndex = getItemIndex(index);
          ...
          if (_hasGroup() && index.isEven) {
            final currentGroup = widget.groupBy!(widget.items![actualIndex]);
            final previousGroup = (actualIndex - 1).isNegative
                ? null
  ...
  void _sortList(List<E?>? list) {
    ...
    final isAscending = widget.order == GroupedListViewOrder.ascending ? 1 : -1;

    list.sort(
      (E? firstItem, E? secondItem) => widget.sortPredicate == null
          ? defaultCompareFunction(firstItem as E, secondItem as E) *
              isAscending
          : widget.sortPredicate!(firstItem as E, secondItem as E) *
              isAscending,
    );
  }
  ...
  List<double> _getGroupHeights() {
    ...
    final groupHeights = groupBy<E?, G>(widget.items!, widget.groupBy!)
        .entries
        .map<double>((entry) => entry.value.length.toDouble())
        .toList();
  ...
  void _scrollControllerListener() {
    final groupHeights = _getGroupHeights();
    final controllerOffset = _scrollController!.offset +
        (widget.headerChangeThreshold ?? _groupHeight!);

    if (controllerOffset < groupHeights.first) {
      _changeCurrentGroupIndex(0);
    } else {
      for (var i = 1; i < groupHeights.length; i++) {
